...
class PhoenixCli : CliktCommand() {
    ...
    private val httpPassword by option(
        "--http-password",
        help = "Password for the http api (default: reads from $confFile)"
    ).required()
...
abstract class PhoenixCliCommand(val name: String, val help: String, printHelpOnEmptyArgs: Boolean = false) :
...
class EstimateLiquidityFees : PhoenixCliCommand(
    name = "estimateliquidityfees",
    help = "Estimates the liquidity fees for a given amount, at current feerates."
) {
...
class CreateInvoice :
    PhoenixCliCommand(name = "createinvoice", help = "Create a Lightning invoice", printHelpOnEmptyArgs = true) {
...
class GetOffer : PhoenixCliCommand(
    name = "getoffer",
    help = "Return a default Lightning offer (reusable invoice). Consider using 'createinvoice' instead."
) {
    override suspend fun httpRequest() = commonOptions.httpClient.use {
...
class GetLnAddress :
    PhoenixCliCommand(name = "getlnaddress", help = "Return a BIP-353 Lightning address (there must be a channel)") {
    override suspend fun httpRequest() = commonOptions.httpClient.use {
...
class PayInvoice :
    PhoenixCliCommand(name = "payinvoice", help = "Pay a Lightning invoice", printHelpOnEmptyArgs = true) {
    private val amountSat by option("--amountSat").long()
...
class PayLnAddress : PhoenixCliCommand(
    name = "paylnaddress",
    help = "Pay a Lightning address (BIP353 or LNURL)",
...
class DecodeInvoice :
    PhoenixCliCommand(name = "decodeinvoice", help = "Decode a Lightning invoice", printHelpOnEmptyArgs = true) {
    private val invoice by option("--invoice").required().check { Bolt11Invoice.read(it).isSuccess }
    override suspend fun httpRequest() = commonOptions.httpClient.use {
        it.submitForm(
...
class DecodeOffer :
    PhoenixCliCommand(name = "decodeoffer", help = "Decode a Lightning offer", printHelpOnEmptyArgs = true) {
...
class LnurlWithdraw : PhoenixCliCommand(
    name = "lnurlwithdraw",
    help = "Withdraw funds from a LNURL service",
    printHelpOnEmptyArgs = true
) {
    private val lnurl by option("--lnurl").required()
...
class LnurlAuth :
    PhoenixCliCommand(name = "lnurlauth", help = "Authenticate on a LNURL service", printHelpOnEmptyArgs = true) {
    private val lnurl by option("--lnurl").required()
...
class SendToAddress :
    PhoenixCliCommand(name = "sendtoaddress", help = "Send to a Bitcoin address", printHelpOnEmptyArgs = true) {
    private val amountSat by option("--amountSat").long().required()
    private val address by option("--address").required()
...
class BumpFee : PhoenixCliCommand(
    name = "bumpfee",
    help = "Bump the fee of an outgoing on-chain transaction",
    printHelpOnEmptyArgs = true
) {
    private val feerateSatByte by option("--feerateSatByte").int().required()
    override suspend fun httpRequest() = commonOptions.httpClient.use {
...
fun String.toByteVector32(): ByteVector32 = kotlin.runCatching { ByteVector32.fromValidHex(this) }
    .recover { error("'$this' is not a valid 32-bytes hex string") }.getOrThrow()